...
ggplot(p,aes(month,fill=p$department))+geom_bar()
...
neighbor
neighbor[1:5]
n1<-subset(s,neighbor="Dorchester")
n2<-subset(s,neighbor="Roxbury")
n3<-subset(s,neighbor="Allston/Brighton")
n4<-subset(s,neighbor="Jamaica")
n5<-subset(s,neighbor="waterfront")
